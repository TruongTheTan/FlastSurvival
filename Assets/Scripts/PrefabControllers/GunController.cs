...
public class GunController : MonoBehaviour
{
	...
	private readonly PlayableCharacterController _playableCharacterController;
	...
	private void OnTriggerEnter2D(Collider2D collision)
	{
		...
		if (isCollideToEnemy && GetComponent<SpriteRenderer>().sprite.name == DataPreserve.SWORD_SPRITE.name)
		{
			...
			bool closeCombatScriptIsEnabled = closeCombatEnemyController.enabled && rangedEnemyController.enabled == false;
			bool rangedCombatScriptIsEnabled = closeCombatEnemyController.enabled == false && rangedEnemyController.enabled;




			if (closeCombatScriptIsEnabled)
